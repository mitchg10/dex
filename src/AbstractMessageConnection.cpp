...
AbstractMessageConnection::AbstractMessageConnection(
  ...
  send_cq = ibv_create_cq(ctx.ctx, 512, NULL, NULL, 0);
...
void AbstractMessageConnection::initRecv() {
  subNR = messageNR / kBatchCount; // 32; messageNR = 32 * 4 = 128
  ...
  // Ready to receive the requests from the remote compute nodes
